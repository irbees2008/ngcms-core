  ...
  <body bgcolor="#ffffff" text="#000000">
    <table border="0" width="100%" cellspacing="1" cellpadding="3">
      <tbody>
        <tr>
          <td width="100%">
            ...
            <div style="border-top: 1px solid #ccc; width: 100%;">
              <br />
              <small>
                {{ news.date }} {{ lang['by'] }} {% if
                pluginIsActive('uprofile') %}<a href="{{ news.author.url }}"
                  >{% endif %} {{ news.author.name }} {% if
                  pluginIsActive('uprofile') %}</a
                >{% endif %}
              </small>
            </div>
          </td>
        ...
        <tr>
          <td width="100%">{{ news.short }} {{ news.full }}</td>
        </tr>
      </tbody>
    </table>
  </body>
