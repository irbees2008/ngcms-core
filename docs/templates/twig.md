...
============================================
Все шаблоны в актуальных версиях NGCMS обрабатываются напрямую через <span class="twig">TWIG</span>, синтаксис доступен сразу без дополнительных маркеров.
Исторически существовали «обычные» шаблоны с включениями кода внутри конструкций `[TWIG] .. [/TWIG]`. Эта конструкция больше не поддерживается: просто удалите маркеры и оставьте чистый Twig‑код.
Пример миграции:
...
... html ...
[TWIG]
{% if isLogged %}Привет, {{ global.user.name }}{% endif %}
[/TWIG]
... html ...
Новый вариант:
... html ...
...
Проверка: если в шаблоне остались маркеры `[TWIG]` или `[/TWIG]`, их следует удалить — иначе они будут интерпретированы как обычный текст.
Глобальные функции
==================
...
Пример, выводящий строку '_ плагин tags активен _' в случае, если плагин tags включен в админ-панели:<br />
...
<b>lang</b> - глобальная переменная с языковыми константами, аналогична конструкции <b>{l\_<span class="blue">Variable</span>}</b> в старом шаблонизаторе.<br />
Пример:
