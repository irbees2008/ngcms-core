...
(function () {
  ...
  // SCEditor toggle button (alternate editor for BBCode)
  function addSCEditorToggleButton() {
    var toolbar = null;
    try {
      toolbar = document.getElementById("tags");
    } catch (e) {}
    ...
    if (!toolbar) return;
    if (window.__ng_sceditor_btn_added) return;
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = "btn btn-outline-dark";
    btn.title = "Переключить на SCEditor (BBCode)";
    btn.innerHTML = '<i class="fa fa-exchange"></i> PM→SC';
    ...
    btn.addEventListener("click", async function (ev) {
      ...
      if (!areaId) areaId = guessAreaId();
      // Load SCEditor integration script if not loaded
      try {
        if (!window.NGCMS || !window.NGCMS.attachSCEditor) {
          var s = document.createElement("script");
          s.src = "/lib/editor_sceditor.js";
          ...
          s.onload = async function () {
            if (window.NGCMS && window.NGCMS.attachSCEditor) {
              await window.NGCMS.attachSCEditor(areaId);
              notifyMsg("SCEditor активирован", "success");
              try {
                if (window.localStorage)
                  localStorage.setItem("ngcms_editor", "sceditor");
              } catch (e) {}
            } else notifyMsg("SCEditor не загрузился", "danger");
          };
          document.head.appendChild(s);
        } else {
          await window.NGCMS.attachSCEditor(areaId);
        ...
        notifyMsg("Ошибка SCEditor: " + e, "danger");
      }
    });
    toolbar.appendChild(btn);
    window.__ng_sceditor_btn_added = true;
  }
