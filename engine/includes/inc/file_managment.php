...
function manage_editApply($type, $id)
{
    ...
    if (isset($_POST['back']) && $_POST['back']) {
        $back = $_POST['back'];
        // Безопасность: разрешаем только относительный путь внутри админки
        if (preg_match('#^\?mod=images#', $back)) {
            // Добавим параметр для отображения сообщения
            $sep = (str_contains($back, '&')) ? '&' : '&';
            // Просто добавляем &iedited=1 (back уже содержит ?mod=images)
            if (!str_contains($back, 'iedited=')) {
                $back .= '&iedited=1';
            }
    ...
    // Фолбэк: если нет back, остаёмся на форме и показываем сообщение сразу
