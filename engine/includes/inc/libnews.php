...
function news_showone($newsID, $alt_name, $callingParams = [])
{
    ...
    if ($callingParams['setCurrentNews']) {
        ...
        $SYSTEM_FLAGS['news']['db.categories'] = [];
    ...
    foreach ($row['#images'] as $k => $v) {
        if ($v['linked_id'] == $row['id']) {
            ...
            $tvars['vars']['_images'][] = [
                'plugin'      => $v['plugin'],
                'pidentity'   => $v['pidentity'],
                'url'         => ($v['storage'] ? '/uploads/dsn' : $config['images_url']) . '/' . $v['folder'] . '/' . $v['name'],
                'purl'        => $v['preview'] ? (($v['storage'] ? '/uploads/dsn' : $config['images_url']) . '/' . $v['folder'] . '/thumb/' . $v['name']) : null,
                'width'       => $v['width'],
                'height'      => $v['height'],
                'pwidth'      => $v['p_width'],
                'pheight'     => $v['p_height'],
...
function news_showlist($filterConditions = [], $paginationParams = [], $callingParams = [])
{
    ...
    $query['filter'] = newsProcessFilter(['AND', ['DATA', 'approve', '=', '1'], $filterConditions]);
    ...
    $showNumber = ($config['number'] >= 1) ? $config['number'] : 8;
    ...
    if (!in_array($orderBy, ['id desc', 'id asc', 'postdate desc', 'postdate asc', 'title desc', 'title asc'])) {
        $orderBy = 'id desc';
    }
    ...
    foreach ($selectResult as $row) {
        ...
        $tvars = newsFillVariables($row, 0, isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 0, 0, isset($callingParams['regenShortNews']) ? $callingParams['regenShortNews'] : []);
        ...
        if (isset($callingParams['overrideTemplatePath']) && $callingParams['overrideTemplatePath']) {
            $templatePath = $callingParams['overrideTemplatePath'];
        } elseif (isset($callingParams['customCategoryTemplate']) && $callingParams['customCategoryTemplate']) {
            // -> check for custom category templates
            // Check mode:
            // 1 - Master category
            // 2 - Current category
    ...
    if (!$nCount) {
        if (!isset($callingParams['extendedReturn']) || !$callingParams['extendedReturn']) {
            msg(['type' => 'info', 'info' => $lang['msgi_no_news']]);
        }
        $limit_start = 2;
    }
