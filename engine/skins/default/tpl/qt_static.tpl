...
<div id="tags" class="btn-toolbar mb-3" role="toolbar">
	...
	<div class="btn-group btn-group-sm mr-2">
		...
		<div class="dropdown-menu" aria-labelledby="tags-block">
			...
			<a href="#" class="dropdown-item" onclick="insertext('[hide]','[/hide]', {{ area }})"><i class="fa fa-shield"></i> {{ lang['tags.hide'] }}</a>
		</div>
	</div>
	<div class="btn-group btn-group-sm mr-2">
		...
		<div class="dropdown-menu" aria-labelledby="tags-link">
			<a href="#" class="dropdown-item" data-toggle="modal" data-target="#modal-insert-url" onclick="prepareUrlModal({{ area }})"><i class="fa fa-link"></i> {{ lang['tags.link'] }}</a>
			<a href="#" class="dropdown-item" data-toggle="modal" data-target="#modal-insert-email" onclick="prepareEmailModal({{ area }})"><i class="fa fa-envelope-o"></i> {{ lang['tags.email'] }}</a>
			<a href="#" class="dropdown-item" data-toggle="modal" data-target="#modal-insert-image" onclick="prepareImgModal({{ area }})"><i class="fa fa-file-image-o"></i> {{ lang['tags.image'] }}</a>
		</div>
	</div>
	<div class="btn-group btn-group-sm mr-2">
	...
	<div class="btn-group btn-group-sm mr-2">
		<button onclick="try{document.forms['DATA_tmp_storage'].area.value={{ area }};} catch(err){;} window.open('{{ php_self }}?mod=files&amp;area={{ area }}', '_Addfile', 'height=600,resizable=yes,scrollbars=yes,width=800');return false;" target="DATA_Addfile" type="button" class="btn btn-outline-dark" title="{{ lang['tags.file'] }}">
			<i class="fa fa-file-text-o"></i>
		</button>
		<button onclick="try{document.forms['DATA_tmp_storage'].area.value={{ area }};} catch(err){;} window.open('{{ php_self }}?mod=images&amp;area={{ area }}', '_Addimage', 'height=600,resizable=yes,scrollbars=yes,width=800');return false;" target="DATA_Addimage" type="button" class="btn btn-outline-dark" title="{{ lang['tags.image'] }}">
...
function prepareUrlModal(areaId){
	try { document.getElementById('urlAreaId').value = areaId; } catch(e){}
	var ta = null;
	try { ta = document.getElementById(areaId); } catch(e){}
	if(!ta){ return; }
	var selText = '';
	...
	if(looksLikeUrl && urlHref && !urlHref.value){ urlHref.value = selText.trim(); }
}
...
function insertEmailFromModal(){
	...
	insertAtCursor(areaId, bb);
	try { $('#modal-insert-email').modal('hide'); } catch(e){
		var modal = document.getElementById('modal-insert-email');
		if(modal){ modal.classList.remove('show'); modal.style.display='none'; }
	}
}
function prepareImgModal(areaId){
	try { document.getElementById('imgAreaId').value = areaId; } catch(e){}
	var ta = null;
	try { ta = document.getElementById(areaId); } catch(e){}
	if(!ta){ return; }
	var selText = '';
	...
	if(looksLikeImg && hrefField && !hrefField.value){ hrefField.value = selText.trim(); }
	if(altField && !looksLikeImg){ altField.value = selText || altField.value || ''; }
}
function insertImgFromModal(){
	...
	if(align){ attrs += ' align="' + align.replace(/[^a-z]/ig,'').toLowerCase() + '"'; }
	var bb = '[img' + attrs + ']' + (alt || '') + '[/img]';
	insertAtCursor(areaId, bb);
	try { $('#modal-insert-image').modal('hide'); } catch(e){
...
function prepareMediaModal(areaId){
	try { document.getElementById('mediaAreaId').value = areaId; } catch(e){}
	var ta = null;
	try { ta = document.getElementById(areaId); } catch(e){}
	if(!ta){ return; }
	var selText = '';
...
function insertMediaFromModal(){
	...
	if(!href){ document.getElementById('mediaHref').focus(); return; }
	var attrs = '';
	...
	insertAtCursor(areaId, bb);
	try { $('#modal-insert-media').modal('hide'); } catch(e){
