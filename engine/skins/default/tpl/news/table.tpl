...
<input type="text" id="dr1" name="dr1" value="{{ dr1 }}" class="form-control" pattern="[0-9]{2}\.[0-9]{2}\.[0-9]{4}" placeholder="{{ "now" | date('d.m.Y G:i:s') }}" autocomplete="off"/>
					...
					<div class="col-lg-4">
						...
						<div class="form-group">
							...
							<select name="sort" class="custom-select">{{ sortlist }}
								<!-- сортировка по редактированию уже добавлена через sortlist -->
							</select>
						</div>
						<div class="form-group">
						...
						<tr>
							...
							<td width="48" nowrap>
								...
								{% if (entry.images_count > 0) %}
									<i class="fa fa-images" title="{{ lang['images.count'] }}: {{ entry.images_count }}"></i>
								{% endif %}
							</td>
							<td nowrap>
