...
<div
	...
	<!-- Открыть модалку быстрой загрузки (изображения/файлы) -->
	<div class="btn-group btn-group-sm mr-2">
		<button type="button" class="btn" title="Загрузить" onclick="try{window.__editorAreaId={{ area }};}catch(e){}; showModalById('modal-uplimg'); return false;">
			<i class="fa fa-folder-open-o"></i>
		</button>
	</div>
	<div class="btn-group btn-group-sm mr-2">
		<button type="button" class="btn" title="{{ lang['tags.image'] }}" data-bs-toggle="modal" data-bs-target="#modal-insert-image" onclick="prepareImgModal({{ area }}); showModalById('modal-insert-image'); return false;">
			<i class="fa fa-picture-o"></i>
		</button>
		<button type="button" class="btn" title="{{ lang['tags.link'] }}" data-bs-toggle="modal" data-bs-target="#modal-insert-url" onclick="prepareUrlModal({{ area }}); showModalById('modal-insert-url'); return false;">
			<i class="fa fa-link"></i>
		</button>
		<button type="button" class="btn" title="{{ lang['tags.email'] }}" data-bs-toggle="modal" data-bs-target="#modal-insert-email" onclick="prepareEmailModal({{ area }}); showModalById('modal-insert-email'); return false;">
			<i class="fa fa-envelope"></i>
		</button>
		{% if pluginIsActive('bb_media') %}
			<button type="button" class="btn" title="[media]" data-bs-toggle="modal" data-bs-target="#modal-insert-media" onclick="prepareMediaModal({{ area }}); showModalById('modal-insert-media'); return false;">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 id="url-modal-label" class="modal-title">{{ lang['tags.link'] }}</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body">
				<input type="hidden" id="urlAreaId" value=""/>
				<div class="mb-3">
					<label for="urlHref" class="form-label">URL</label>
					<input type="text" class="form-control" id="urlHref" placeholder="https://example.com"/>
				</div>
				<div class="mb-3">
					<label for="urlText" class="form-label">{{ lang['tags.link_text']|default('Текст ссылки') }}</label>
					<input type="text" class="form-control" id="urlText" placeholder="{{ lang['tags.link_text']|default('Текст ссылки') }}"/>
				</div>
				<div class="row">
					<div class="col-md-6 mb-3">
						<label for="urlTarget" class="form-label">Target</label>
						<select id="urlTarget" class="form-select">
							<option value="">--</option>
							<option value="_blank">_blank</option>
						</select>
					</div>
					<div class="col-md-6 mb-3 form-check" style="padding-left: 2.25rem;">
						<input type="checkbox" class="form-check-input" id="urlNofollow"/>
						<label class="form-check-label" for="urlNofollow">rel="nofollow"</label>
					</div>
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ lang['btn_cancel'] }}</button>
				<button type="button" class="btn btn-primary" onclick="insertUrlFromModal()">{{ lang['btn_insert'] }}</button>
			</div>
		</div>
	</div>
</div>
<!-- Modal: Insert Email (BS5) -->
<div id="modal-insert-email" class="modal fade" tabindex="-1" aria-labelledby="email-modal-label" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 id="email-modal-label" class="modal-title">{{ lang['tags.email'] }}</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body">
				...
				<div class="mb-3">
					<label for="emailText" class="form-label">{{ lang['tags.link_text']|default('Текст') }}</label>
					<input type="text" class="form-control" id="emailText" placeholder="{{ lang['tags.link_text']|default('Текст') }}"/>
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ lang['btn_cancel'] }}</button>
				<button type="button" class="btn btn-primary" onclick="insertEmailFromModal()">{{ lang['btn_insert'] }}</button>
			</div>
		</div>
	</div>
</div>
<!-- Modal: Insert Image (BS5) -->
<div id="modal-insert-image" class="modal fade" tabindex="-1" aria-labelledby="img-modal-label" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 id="img-modal-label" class="modal-title">{{ lang['tags.image'] }}</h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body">
				<input type="hidden" id="imgAreaId" value=""/>
				<div class="mb-3">
					<label for="imgHref" class="form-label">URL</label>
				...
				<div class="mb-3">
					...
					<input type="text" class="form-control" id="imgAlt" placeholder="Подпись"/>
				</div>
				<div class="row">
					<div class="col-md-4 mb-3">
						<label for="imgWidth" class="form-label">Ширина</label>
						<input type="number" min="0" class="form-control" id="imgWidth" placeholder="Напр. 640"/>
					</div>
					<div class="col-md-4 mb-3">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ lang['btn_cancel'] }}</button>
				<button type="button" class="btn btn-primary" onclick="insertImgFromModal()">{{ lang['btn_insert'] }}</button>
			</div>
		</div>
	</div>
</div>
			...
			<div class="modal-body">
				<ul class="nav nav-tabs" role="tablist">
					<li class="nav-item" role="presentation">
						<button class="nav-link active" id="upl-img-tab" data-bs-toggle="tab" data-bs-target="#upl-img" type="button" role="tab" aria-controls="upl-img" aria-selected="true">Загрузка изображений</button>
					</li>
					<li class="nav-item" role="presentation">
						<button class="nav-link" id="upl-data-tab" data-bs-toggle="tab" data-bs-target="#upl-data" type="button" role="tab" aria-controls="upl-data" aria-selected="false">Загрузка файла</button>
					</li>
				</ul>
				<div class="tab-content mt-3">
					<div id="upl-img" class="tab-pane fade show active" role="tabpanel" aria-labelledby="upl-img-tab">
						<ul id="newsimage-area" class="list-unstyled"></ul>
					...
					<div id="upl-data" class="tab-pane fade" role="tabpanel" aria-labelledby="upl-data-tab">
						<ul id="newsfile-area" class="list-unstyled"></ul>
						<div class="clearfix"></div>
						<div class="mb-2"></div>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-4 mb-3">
							...
							<input type="number" min="0" class="form-control" id="mediaWidth" placeholder="640"/>
						</div>
						<div class="col-md-4 mb-3">
							<label for="mediaHeight" class="form-label">{{ lang['media.height'] }}</label>
							<input type="number" min="0" class="form-control" id="mediaHeight" placeholder="360"/>
						</div>
						<div class="col-md-4 mb-3">
